...
public class HealBerry extends ItemFood implements IPlantable{
    private String name;
    public HealBerry(String name, int amount, boolean isWolfFood) {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hx, float xy, float xz){
        ...
        if(facing == EnumFacing.UP && player.canPlayerEdit(pos.offset(facing), facing, stack) && state.getBlock().canSustainPlant(state, world, pos, EnumFacing.UP, this) && world.isAirBlock(pos.up())){
            world.setBlockState(pos.up(), SBlocks.heal_crop.getDefaultState());
            stack.shrink(1);
    ...
    public IBlockState getPlant(IBlockAccess world, BlockPos pos) {
        return SBlocks.heal_crop.getDefaultState();
    }

    @Override
    protected void onFoodEaten(ItemStack stack, World worldIn, EntityPlayer player) {
        if (!worldIn.isRemote){
            player.addPotionEffect(new PotionEffect(MobEffects.REGENERATION, 10*20, 5));
        }
    }
}
